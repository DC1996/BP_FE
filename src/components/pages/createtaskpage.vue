<!-- 
 * File: createtaskpage.vue
 * Description: Implements task creation
 *
...
<template>
    <v-container
        ...
        <v-container fluid class="d-flex flex-row flex-grow-1">
            ...
            <h3 class="ml-2 mb-1"> Variables </h3>          
                <div class="d-flex flex-row flex-wrap align-center ma-0 pa-0 minus">
                    <div v-for="(variable, ind) in extracted.task.variables" :key="ind + 'v'" class="ma-0 pa-0">
                ...
                <h3 class="ml-2">Answers</h3>
                    <div class="d-flex flex-row flex-wrap align-center ma-0 pa-0 minus">
                        <p 
                            ...
                            v-if="extracted.task.answers.length == 0" >
                            No Answers found. Add Answers to questions.
                        </p>
                        <div v-for="(answer, i) in extracted.task.answers" :key="i + 'a'" class="ma-0 pa-0">
                            <div class="mx-2 my-1" v-if="variable.visible">
                        ...
                        <div 
                            ...
                            <div v-if="selectedPreviewRenderOption == 'select'" class="d-flex flex-row justify-space-between mb-1"> 
                                <div v-for="(answer, index) in question.answers" :key="index"
                                    ...
                                    <v-select style="max-width: 200px"
                                        ...
                                        :rules="[v => preview.expectedAnswers.find(({name}) => name == answer.name).value == answer.selected || 'Wrong answer' ]"
                            ...
                            <div v-if="selectedPreviewRenderOption == 'radio'" class="d-flex flex-row justify-space-between mb-1">  
                                <div class="d-flex flex-column" v-for="(answer, index) in question.answers" :key="index">
                                    <v-radio-group v-model="answer.selected" :rules="[v => preview.expectedAnswers.find(({name}) => name == answer.name).value == v || 'Wrong answer' ]">
                                        <v-radio dense class="py-1"                                        
                                            v-for="(value, i) in answer.options"
                                            :key="value.toString() + i" :label="value.toString()" :value="value">
                                        </v-radio>
                                    </v-radio-group>
                                </div>
                            </div>
                            ...
                            <div class="d-flex flex-row justify-space-between mb-1" v-if="selectedPreviewRenderOption == 'checkbox'"> 
                                <div class="d-flex flex-column py-1 px-2" v-for="(answer, index) in question.answers" :key="index">
                                    <v-checkbox class="pa-0 ma-0"
                                        v-for="(value, i) in answer.options"
                                        :color="preview.expectedAnswers.find(({name}) => name == answer.name).value == value ? 'primary' : 'error'"
                            ...
                            <div class="d-flex flex-row justify-space-between mb-1" v-if="selectedPreviewRenderOption == 'text'">
                                <div class="dflex flex-column py-1 mr-2" v-for="(answer, index) in question.answers" :key="index">
                                    <v-text-field 
                                        :rules="[v => preview.expectedAnswers.find(({name}) => name == answer.name).value == v || 'Wrong answer']"
                                        dense label="Answer" outlined>
                                    </v-text-field>
                                </div>
                            </div>
                    ...
                    <div class="d-flex flex-column justify-start">
                        <v-form ref="qForm" style="width: 100%">
                            <div
                                ...
                                v-for="(question, i) in extracted.task.questions" :key="i">
                                ...
                                <v-text-field
                                    ...
                                    v-model="extracted.task.questions[i].text">
        ...
        <v-overlay tabindex="0" @keydown.esc="overlayV = false"
            :value="overlayV" :light="true" :dark="false" v-if="extracted.task.variables.length != 0">
            <v-sheet 
        ...
        <v-overlay
            ...
            :value="overlayA" v-if="extracted.task.answers.length != 0">
...
<script>
    import { tokenize, Token } from "../../languageHandlers/tokenizer.js"
    ...
    export default {
        data() {   
            return {
                ...
                variableRules: [
                    ...
                    v => this.extracted.task.variables.filter((_, index) => this.indexV !== index).find( variable => variable.name == '$'.concat(v)) == undefined || 'Variable name already exists'
                ],
                ...
                TaskDataService.get(localStorage.getItem('taskId'))
                    .then((res) => {
                        ...
                        try {
                            ...
                            this.extracted.task = parseMathLang(res.data.content);
                        ...
                        this.text = this.extracted.task.text.trim();
        ...
        methods: {
            ...
            testext() {
                // Test extraction
                this.extracted.extractVariable(tokenize(this.text));
            },

            async createNewTask() 
            {
                // Prepare data for sending
                var data = { name: this.taskName, grade: this.taskGrade, content: this.extracted.generateText() };
            ...
            variableInfo : function(i) {
                this.indexV = i;
                this.variable = cloneDeep(this.extracted.task.variables[i]);
            ...
            setVariableInfo: function() {
                ...
                this.extracted.task.variables[this.indexV] = cloneDeep(this.variable);
                this.overlayV = false;
            ...
            answerInfo: function(i) {
                this.indexA = i
                this.answer = cloneDeep(this.extracted.task.answers[i]);
                this.overlayA = true;
            },
            // close and save edited values
            setAnswerInfo: function() {
                this.extracted.task.answers[this.indexA] = cloneDeep(this.answer);
                this.overlayA = false;
            ...
            previewTask: function () {
                ...
                GeneratorService
                    .generate( { content: this.extracted.task } )
                    .then((res) => {
                        ...
                        localStorage.setItem('data', this.extracted.generateText() );
                    })
                    .catch((err) => {
            ...
            addVariable: function() {
                ...
                this.extracted.task.variables.push(newVariable);
            },
            removeVariable: function(i) {
                this.extracted.task.variables.splice(i, 1);
            },

            // Questions
            addQuestion: function() {
                this.extracted.task.questions.push(new Question());
            },
            removeQuestion: function(i) {
                this.extracted.task.questions.splice(i, 1);
            },

            // Answers
            removeAnswer(i) {
                this.extracted.task.answers.splice(i, 1);
            },
            addCorrect: function() {
            ...
            parseText : function () {
                ...
                // VARIABLES
                this.variables = this.extracted.extractVariables(tokens);
                ...
                try{
                    let generated_text = this.extracted.generateText();
